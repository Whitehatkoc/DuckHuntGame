...
public class LevelSix {
    ...
    private double imageSpeedYBlue;
    private final ArrayList<ImageView> duckList = new ArrayList<>();
public Scene getLevelSixScene(Stage primaryStage, ImageCursor selectedCursor, ImageView selectedBackground, Integer countForBackGround, ImageView foreGroundNode){

    // it belongs to the last level, the ending music of the game
    MediaPlayer gameCompletedMusic =new MediaPlayer(new Media(new File("assets\\effects\\GameCompleted.mp3").toURI().toString()));
    gameCompletedMusic.setVolume(DuckHunt.VOLUME);

    // prepare th selection pane
    Pane selectionPane = new Pane();
    selectionPane.getChildren().add(selectedBackground);
    Scene levelSixScene = new Scene(selectionPane);

    // screen information texts
    Label levelText = new Label("Level: 6/6");
    ...
    // setting the selected cursor as a cursor
    EventHandler<MouseEvent> cursorHandler = event -> {
        ...
        double sceneWidth = levelSixScene.getWidth();
        double sceneHeight = levelSixScene.getHeight();

        // Check the scene boundaries
        if (sceneX <= 0 || sceneX >= sceneWidth || sceneY <= 0 || sceneY >= sceneHeight) {
            levelSixScene.setCursor(Cursor.DEFAULT);
        } else {
            levelSixScene.setCursor(selectedCursor);
        }
    };
    ...
    for(int i =0; i<3;i++) {
        ...
        //the visual dimensions of the bird are recorded for later use
    ...
    // movement of the red bird, cross-flying
    ...
    // movement of the blue bird
    int imageAngleBlue=50;
    imageSpeedXBlue = DuckHunt.SCALE *2*Math.cos(Math.toRadians(imageAngleBlue));
    imageSpeedYBlue = -(DuckHunt.SCALE) *2*Math.sin(Math.toRadians(imageAngleBlue));
    AnimationTimer animationTimerBlue = new AnimationTimer() {
        @Override
        public void handle(long now) {

            imageXBlue += imageSpeedXBlue;
            imageYBlue += imageSpeedYBlue;
            ...
            if (Math.round(imageYBlue)< 0 || Math.round(imageYBlue + heightDucks*DuckHunt.SCALE) > DuckHunt.Height) {
                imageSpeedYBlue = -imageSpeedYBlue;
                duckNodeBlue.setScaleY(-duckNodeBlue.getScaleY());
            }

            // update the location of the bird
            duckNodeBlue.setTranslateX(imageXBlue);
            duckNodeBlue.setTranslateY(imageYBlue);
        }
    };
                    ...
                    selectionPane.setOnKeyPressed(keyEvent->{
                        switch (keyEvent.getCode()){
                            case ENTER:
                                ...
                                FirstLevel.duckFallsMusic.stop();
                                FirstLevel.gunShotMusic.stop();
                                //if the user wants, the first level is started
                            ...
                            case ESCAPE:
                                ...
                                FirstLevel.duckFallsMusic.stop();
                                FirstLevel.gunShotMusic.stop();
                                // if the user wants, it is entered into the login screen
    ...
    return levelSixScene;

}

    /**
     * Returns a VBox containing the text for "game winning" message.
     *
     * @return The VBox containing the winning message text.
     */
    public VBox winnerText(){
        Text text1 = new Text("You have completed the game!");
